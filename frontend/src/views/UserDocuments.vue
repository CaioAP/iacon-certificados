    ...
    export default {
            ...
            tableActions() {
                return [
                    ...
                    },
                    {
                        ...
                        action: () => {},
                        title:
                            'Clique aqui para visualizar o protocolo dos arquivos',
                    },
                    {
                        id: 'comment',
                        type: 'icon',
        ...
        methods: {
            ...
            getPeriodName(periodValue) {
                const period = this.periodOptions.filter(
                ...
                values.forEach((val) => {
                    for (const key in val) {
                        tableDataInit.oblied.forEach((data) =>
                            data[key] == val[key]
                                ? tableData.oblied.push(data)
                                : null
                        )
                        tableDataInit.extra.forEach((data) =>
                            data[key] == val[key]
                                ? tableData.extra.push(data)
                                : null
                        )
                    }
                })
            ...
            getUserFiles() {
                const companyIds =
                    '?companyIds=' +
                    this.userData.companies.map((data) => data[0]).join(',')
                            ...
                            if (
                                ...
                                row.hasfile =
                                    row.companyId in documents
                                        ? row.documentPath in
                                              documents[row.companyId] &&
                                          documents[row.companyId][row.documentPath]
                                              .length > 0
                                            ? true
                                            : false
                                        : false
                ...
                axios
                    ...
                    .then((response) => {
                        ...
                        response.data.result.forEach((message) => {
                            ...
                            message.myMessage =
                                message.usertype === messageModal.usertype
                                    ? true
                                    : false
            ...
            sendMessage(event) {
                ...
                axios
                    .post(`http://${location.hostname}:2160/users/message`, data)
                    .then((response) => {
